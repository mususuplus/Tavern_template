import { ArrowRight, Coins, Compass, Shield, Sparkles, User } from 'lucide-react';
...
const ABILITY_DESCRIPTIONS: Record<string, string> = {
  ...
  传奇法师:
    '能够开辟自定法则的独立半位面，感知笼罩百里疆域并在挥手间禁绝一切敌对魔法，其终极之威足以一击抹除整座要塞。',
...
const CustomizationPage: React.FC<CustomizationPageProps> = ({ onComplete }) => {
  ...
  const getAbilityDescription = (profession: string, faith: string) => {
    ...
    if (['受感者', '祭司', '大祭司', '神使'].includes(profession)) {
      if (faith === '无') return '你的神明将赐予你独一无二的能力。(请在上一步确立信仰)';
        ...
        <div className="w-full md:w-72 bg-ink-500 p-8 text-parchment-50 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-6">
            {steps.map((s, i) => (
              <div
                key={i}
                className={`flex items-center gap-4 transition-all duration-300 ${i === step ? 'translate-x-2' : 'opacity-40'}`}
              >
                <div
                  className={`w-10 h-10 rounded-full border flex items-center justify-center ${i === step ? 'bg-rust-600 border-rust-500 text-white' : 'border-parchment-100/30 text-parchment-200'}`}
                >
                  {i + 1}
                </div>
              ...
              {step === 0 && (
                ...
                >
                  {GODS.map(g => (
                    ...
                    >
                      <div
                        className={`font-display tracking-widest text-lg ${config.faith === g ? 'text-amber-700' : 'text-ink-600'}`}
                      >
                        {g}
                      </div>
              ...
              {step === 2 && (
                ...
                >
                  {LOCATIONS.map(l => (
                    ...
                    >
                      <span
                        className={`font-display tracking-widest ${config.location === l ? 'text-emerald-700' : 'text-ink-600'}`}
                      >
                        {l}
                      </span>
                      {config.location === l && (
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <input
                          ...
                          onChange={e =>
                            setConfig({
                              ...config,
                              currency: { ...config.currency, gold: parseInt(e.target.value) || 0 },
                            })
                          }
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <input
                          ...
                          onChange={e =>
                            setConfig({
                              ...config,
                              currency: { ...config.currency, silver: parseInt(e.target.value) || 0 },
                            })
                          }
                          className="bg-white border border-ink-500/20 p-4 rounded-sm font-mono text-center text-xl"
                        />
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <input
                          ...
                          onChange={e =>
                            setConfig({
                              ...config,
                              currency: { ...config.currency, copper: parseInt(e.target.value) || 0 },
                            })
                          }
                          className="bg-white border border-ink-500/20 p-4 rounded-sm font-mono text-center text-xl"
                        />
          ...
          <div className="mt-12 flex justify-between items-center">
            ...
            {step < steps.length - 1 ? (
              ...
              <button
                ...
                className="px-12 py-4 bg-ink-500 text-parchment-50 rounded-sm font-display tracking-[0.3em] uppercase hover:bg-ink-600 transition-all"
              >
                开启神话旅程
              </button>
            )}
